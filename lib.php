...
function magtest_add_instance($magtest) {
    ...
    $editordata = $magtest->result;
    file_save_draft_area_files(0, $context->id, 'mod_customlabel', 'result', 0);
    $magtest->result = file_rewrite_urls_to_pluginfile($editordata['text'], $editordata['itemid'], 0);

    $magtest->id = $DB->insert_record('magtest', $magtest);
...
function magtest_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload) {
    ...
    $fileareas = array('question', 'questionanswer', 'helper');
