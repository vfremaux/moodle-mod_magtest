...
$string['noanswerusers'] = 'Sans réponse&ensp;';
...
$string['singlechoice_help'] = 'Si actif, seule la première catégorie. Weights apply to all other categories if this questions is enabled. Le test doit être pondéré dans ce cas.';
$string['pagenotcomplete'] = 'Toutes les réponses n\'ont pas été données';
...
$string['singlechoicemode'] = 'Ce test magazine est en mode réponse simple. Vous ne pouvez pas saisir de textes de réponse dans ce cas. L\'étudiant ne peut que répondre "oui" ou "non" à chaque question posée. Les pondérations distribuent des points correspondants aux catégories'; 
...
Le fichier d\import doit être encodé en UTF8, séparé par des ";", une question par ligne et doit
avoir une première colonne avec le texte de la question.

Si l\'option "Choix simple" est active, alors les colonnes suivantes donnent dans l\'ordre les poids de catégorie (entier).

Si l\'option "Choix simple" est désactivée (par défaut), alors les colonnes suivantes viennent par
groupe de trois et donnent dans l\'ordre et pour chaque catégorie,
...
Ce paramètre permet de régler le nombre de questions que vous voulez afficher sur chaque page du test.
Si vous le réglez sur 1, le teste affichera chaque question sur une nouvelle page.
';
...
Ceci permet de créer des tests qui permettent de faire varier l\'influence de certaines questions
dans le "placement" des participants dans le score final.

En mode pondéré, la pondération par défaut est de 1, ce qui revient à ne pas utiliser de pondération';

$string['magtest_help'] = '
Le module "Test de magazine" propose un test qui se base sur un ensemble de catégories de réponses,
comme les tests de psychologie qu l\'on trouve dans les magazines. Le test permet d\'établir les catégories
de classement du résultat, et propose de définir un jeu de questions �oser aux participants. A chaque
question doit correspondre un certain nombre de propositions (une par catégorie) permettant au participant
de faire son choix.

Lorsque le participant fait le test, il devra choisir pour chaque question la réponse appropriée ce qui
rapportera un certain nombre de points à la catégorie correspondante.

A la fin du test, la catégorie gagnante est celle qui a collecté de meilleur score. Une conclusion est
alors montrée aux utilisateurs, laquelle souligne la conclusion "gagnante", suivie éventuellement par une
conclusion plus générale.

Les groupes de participants issus du test et classés par catégorie gagnante peuvent être convertis en groupes
Moodle du cours courant. Ce module peut donc servir comme "discriminateur à critère" pour former des groupes par
l\'action des participants eux-mêmes.
';

$string['usemakegroups_help'] = '
### Génération de groupes Moodle

Le module "Test de magazine" permet de segmenter les participants en plusieurs groupes par leur simple
participation au test et la catégorie de proposition qu\'ils vont choisir par leur réponses.

Le module permet d\'activer la fonction de génération de groupes qui définit des groupes Moodle de cours à
partir de ces résultats.

Le test peut ne pas être complet et les groupes seront générés avec les réponses disponibles. Par contre, il n\'est
pas possible de générer les groupes s\'il existe déjà des groupes définis dans le cours. Vous devrez donc détruire les
groupes après une première tentative si vous désirez à nouveau générer les groupes à partir de nouveaux résultats.
';

$string['allowreplay_help'] = '
### Autoriser plusieurs tentatives

Ce paramètre permet de commuter la possibilité de rejouer le test au niveau de l\'instance. Les utilisateurs
doivent en plus disposer de la capacité "Rejouer le test" dans leur profil pour pouvoir exécuter le test à nouveau.
';
