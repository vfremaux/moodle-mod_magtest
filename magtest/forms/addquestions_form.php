    ...
    function definition(){
       global $DB,$CFG,$cm,$qid,$id,$magtest;
        ...
        if($this->cmd == 'add')
        {
            ...
            $mform->addElement('editor', 'questiontext_editor',get_string('question_text','magtest'),null,array('maxfiles' => EDITOR_UNLIMITED_FILES,
            ...
            $question_editor = $mform->addElement('editor', 'questionanswer'.$cat->id.'_editor',$question_answer_text,null,array('maxfiles' => EDITOR_UNLIMITED_FILES,
                            'noclean' => true, 'context' =>  $mod_context));   
            }
        ...
        else if ($this->cmd == 'update')
        {
            ...
            $questiontext_editor  = $mform->addElement('editor', 'questiontext_editor',get_string('question_text','magtest'),null,array('maxfiles' => EDITOR_UNLIMITED_FILES,
                            'noclean' => true, 'context' =>  $mod_context));
                            
            $question = file_prepare_standard_editor($question, 'questiontext', $questionoptions, $mod_context,
            ...
            foreach ($cats as $cat){        
                ...
                $answer = $DB->get_record_sql ('select * from {magtest_answer} where magtestid='.$magtest->id.' and categoryid='.$cat->id.' and questionid='.$question->id);               
                $question_editor = $mform->addElement('editor', 'questionanswer'.$cat->id.'_editor',$question_answer_text,null,array('maxfiles' => EDITOR_UNLIMITED_FILES,
                            'noclean' => true, 'context' =>  $mod_context));                   
                if(!empty($answer)){
