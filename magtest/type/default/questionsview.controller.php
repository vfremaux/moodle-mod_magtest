...
update_record('magtest_question', $question);
...
foreach($tab_answers as $id=>$answer) {
  ...
  update_record('magtest_answer', (object)$answer);
...
//already done 
 ...
 case get_string('delquestion','magtest') : 
   ...
   if (! record_exists('magtest_useranswer', 'questionid', $question->id)) {
     delete_records('magtest_question', 'id', $question->id);
     delete_records('magtest_answer','questionid',$question->id);
     ...
     for ($i = $question->qorder+1; $i <= $nb_questions; $i++) {
       ...
       update_record('magtest_question', $question2);
     }
     $nb_questions = $nb_questions - 1;
